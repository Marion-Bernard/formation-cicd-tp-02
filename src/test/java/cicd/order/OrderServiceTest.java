...
class OrderServiceTest {
    ...
    class ComputeTotal {
        ...
        void simple_order_no_discount_no_priority() {
            Order order = Order.create("O1", 2, 10.0, false);
        ...
        void order_with_discount_only() {
            Order order = Order.create("O2", 10, 10.0, false); // subtotal 100
            double total = service.computeTotal(order);
        ...
        void order_with_priority_only() {
            Order order = Order.create("O3", 2, 10.0, true); // subtotal 20 + 9.99
            double total = service.computeTotal(order);
        ...
        void order_with_discount_and_priority() {
            Order order = Order.create("O4", 10, 10.0, true); // subtotal 100 -> discount 5% = 95 + 9.99
            double total = service.computeTotal(order);
        ...
        @Test
        void rounding_test() {
            Order order = Order.create("O5", 3, 33.333, true); // subtotal 99.999, no discount, +9.99
            double total = service.computeTotal(order);
        ...
        void edge_discount_threshold() {
            Order order = Order.create("O6", 10, 10.0, false); // subtotal 100
            double total = service.computeTotal(order);
        ...
        void edge_discount_just_below_threshold() {
            Order order = Order.create("O7", 9, 10.0, false); // subtotal 90
            double total = service.computeTotal(order);
